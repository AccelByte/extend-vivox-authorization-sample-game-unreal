...
/**
 * Called when an audio processing unit is about to inform its acoustic echo canceller about what audio is being rendered.
 * Needs to be implemented when audio from pf_on_audio_unit_before_recv_audio_rendered is changed and rendered some way other than through Vivox.
 * Game audio can be mixed into this buffer to attempt cancellation of game audio bleed in addition to voice echo cancellation.
 * No blocking operations should occur on this callback.
 */
typedef void (*pf_on_audio_unit_requesting_final_mixed_recv_audio_t)(
        void *callback_handle,
        const char *session_group_handle,
        const char *initial_target_uri,
        short *pcm_frames,
